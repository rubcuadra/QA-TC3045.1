...
Verifysoft Technology GmbH              macOS 
12 January 2017


This readme.txt file contains macOS platform-specific information about
...
CONTENTS:
  ...
  Uninstalling
  Remark of ctc.h and libctc.a file
...
The binaries in this OSX distribution were built on Linux Mint 18.1 
with i686-apple-darwin10-gcc v4.2.1 compiler.
...
to another machine, which even may not be running macOS, than this macOS
...
(example)
   ...
   make install prefix=/opt/testwell/ctc_v8.1 \
                COMPUTER="OSX 123.44.55.66" \
                EXPIRATION="15 Oct 2016" \
...
Remark of ctc.h and libctc.a files
----------------------------------
...
where the libctc.a reside. When ctc-machinery generates
a link command for the instrumented executable, it specifies the ctc
run-time library file with explicit path. 
...
We assume, that you have install CTC++ in /usr/local ..

Xcode has a reduced PATH settings, we first need to symlink ctc and ctc2dat.

We assume that default clang compiler is used. 

For MacOSX target, it is located here with XCode 5.1.1 in
/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/
That directory is in front of PATH. We need a symlink to ctc in there:
  cd /Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/
  sudo ln -s /usr/local/bin/ctc . 

Then for some reasons, we need ctc2dat in PATH, when programs are executed from Xcode.
The directory is /Applications/Xcode.app/Contents/Developer/usr/bin/
  cd /Applications/Xcode.app/Contents/Developer/usr/bin/ 
  sudo ln -s /usr/local/bin/ctc2dat . 

Then the CTC++ runtime library /usr/local/lib/libctc.a needs to be added manually
to the project. Finder didn't list /usr/local/lib/libctc.a, so we 
copied it to $HOME/Desktop/ and then drag/dropped libctc.a from where to the project.

Now, what's left, we need to wrap the clang compiler.
Go to compiler directory 
  cd /Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/
  sudo mv clang clang-orig 
Then add wrapper file clang with following two lines (or use clang file in installation
package (not installed))
#!/bin/sh 
exec ctc -i m `dirname $0`/clang-orig $*

Thei prefixes call to compiler with ctc and parameters (here -i m for multicondition),
passes path_to/clang-orig (original compiler) then rest of command line.
exec doesn't return to next line, process will be replace with ctc, return value
then as given by ctc

For clang++ nothing then todo, it is already a symlink to clang, which is now the wrapper.
